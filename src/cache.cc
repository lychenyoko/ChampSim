...
void CACHE::readlike_hit(std::size_t set, std::size_t way, PACKET& handle_pkt)
{
  ...
  if (should_activate_prefetcher(handle_pkt.type) && handle_pkt.pf_origin_level < fill_level) {
    ...
    handle_pkt.pf_metadata = impl_prefetcher_cache_operate(pf_base_addr, handle_pkt.ip, handle_pkt.instr_id, 1, handle_pkt.type, handle_pkt.pf_metadata);
  }
...
bool CACHE::readlike_miss(PACKET& handle_pkt)
{
  ...
  if (should_activate_prefetcher(handle_pkt.type) && handle_pkt.pf_origin_level < fill_level) {
    cpu = handle_pkt.cpu;
    uint64_t pf_base_addr = (virtual_prefetch ? handle_pkt.v_address : handle_pkt.address) & ~bitmask(match_offset_bits ? 0 : OFFSET_BITS);
    handle_pkt.pf_metadata = impl_prefetcher_cache_operate(pf_base_addr, handle_pkt.ip, handle_pkt.instr_id, 0, handle_pkt.type, handle_pkt.pf_metadata);
  }
